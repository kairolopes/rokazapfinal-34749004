...
exports.sendLink = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Usuário não autenticado");
    }
    const { phone, message, linkUrl, title, linkDescription, image, conversationId, messageDocId } = data;
    console.log("sendLink - Payload recebido:", JSON.stringify({ phone, message, linkUrl, title, linkDescription, imageLength: image === null || image === void 0 ? void 0 : image.length, conversationId, messageDocId }));
    ...
    const body = { phone, linkUrl };
    if (message)
        body.message = message;
    if (title)
        body.title = title;
    if (linkDescription)
        body.linkDescription = linkDescription;
    if (image)
        body.image = image;
...
//# sourceMappingURL=sendLink.js.map