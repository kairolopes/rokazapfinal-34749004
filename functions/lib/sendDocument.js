...
exports.sendDocument = functions.runWith({ timeoutSeconds: 120, memory: "512MB" }).https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Usuário não autenticado");
    }
    const { phone, fileName, caption, conversationId, messageDocId } = data;
    const document = data.document || "";
    console.log("sendDocument - Payload recebido:", JSON.stringify({ phone, fileName, captionLength: caption === null || caption === void 0 ? void 0 : caption.length, conversationId, messageDocId, documentLength: document === null || document === void 0 ? void 0 : document.length }));
    if (!phone || !document || !conversationId || !messageDocId || !fileName) {
        throw new functions.https.HttpsError("invalid-argument", "phone, document, fileName, conversationId e messageDocId são obrigatórios");
    }
    ...
    const url = `${config.apiUrl}/instances/${config.instanceId}/token/${config.instanceToken}/send-document/${ext}`;
    console.log("sendDocument - URL Z-API:", url);
    const body = { phone, document, fileName };
    if (caption)
    ...
    console.log("sendDocument - Response status:", response.status, "body:", responseText);
    if (!response.ok) {
    ...
    console.log("sendDocument - Sucesso! zaapId:", result.zaapId, "messageId:", zapiMessageId);
