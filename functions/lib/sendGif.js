...
exports.sendGif = functions.runWith({ timeoutSeconds: 120, memory: "512MB" }).https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Usuário não autenticado");
    }
    const { phone, gif, caption, conversationId, messageDocId } = data;
    console.log("sendGif - Payload recebido:", JSON.stringify({ phone, gifLength: gif === null || gif === void 0 ? void 0 : gif.length, caption, conversationId, messageDocId }));
    if (!phone || !gif || !conversationId || !messageDocId) {
        throw new functions.https.HttpsError("invalid-argument", "phone, gif, conversationId e messageDocId são obrigatórios");
    }
    ...
    const url = `${config.apiUrl}/instances/${config.instanceId}/token/${config.instanceToken}/send-gif`;
    console.log("sendGif - URL Z-API:", url);
    const body = { phone, gif };
    if (caption)
    ...
    console.log("sendGif - Response status:", response.status, "body:", responseText);
    if (!response.ok) {
    ...
    console.log("sendGif - Sucesso! zaapId:", result.zaapId, "messageId:", zapiMessageId);
    return { zaapId: result.zaapId, messageId: zapiMessageId };
});
//# sourceMappingURL=sendGif.js.map