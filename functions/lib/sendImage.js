...
exports.sendImage = functions.runWith({ timeoutSeconds: 120, memory: "512MB" }).https.onCall(async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Usuário não autenticado");
    }
    const { phone, caption, conversationId, messageDocId } = data;
    let image = data.image || "";
    console.log("sendImage - Payload recebido:", JSON.stringify({ phone, captionLength: caption === null || caption === void 0 ? void 0 : caption.length, conversationId, messageDocId, imageLength: image === null || image === void 0 ? void 0 : image.length }));
    if (!phone || !image || !conversationId || !messageDocId) {
        throw new functions.https.HttpsError("invalid-argument", "phone, image, conversationId e messageDocId são obrigatórios");
    }
    ...
    const url = `${config.apiUrl}/instances/${config.instanceId}/token/${config.instanceToken}/send-image`;
    console.log("sendImage - URL Z-API:", url);
    const body = { phone, image };
    if (caption)
    ...
    console.log("sendImage - Response status:", response.status, "body:", responseText);
    if (!response.ok) {
    ...
    console.log("sendImage - Sucesso! zaapId:", result.zaapId, "messageId:", zapiMessageId);
