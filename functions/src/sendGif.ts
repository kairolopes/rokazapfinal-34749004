...
export const sendGif = functions.runWith({ timeoutSeconds: 120, memory: "512MB" }).https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  console.log("sendGif - Payload recebido:", JSON.stringify({ phone, gifLength: gif?.length, caption, conversationId, messageDocId }));
  ...
  const body: Record<string, string> = { phone, gif };
