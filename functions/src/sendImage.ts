...
export const sendImage = functions.runWith({ timeoutSeconds: 120, memory: "512MB" }).https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  console.log("sendImage - Payload recebido:", JSON.stringify({ phone, captionLength: caption?.length, conversationId, messageDocId, imageLength: image?.length }));
  ...
  const body: Record<string, string> = { phone, image };
