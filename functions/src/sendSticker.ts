...
export const sendSticker = functions.runWith({ timeoutSeconds: 120, memory: "512MB" }).https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Usuário não autenticado");
  }

  const { phone, conversationId, messageDocId } = data;
  let sticker: string = data.sticker || "";

  console.log("sendSticker - Payload recebido:", JSON.stringify({ phone, conversationId, messageDocId, stickerLength: sticker?.length }));
  ...
  const body: Record<string, string> = { phone, sticker };
  if (sticker.startsWith('http')) {
