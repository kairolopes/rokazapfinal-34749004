...
export const sendDocument = functions.runWith({ timeoutSeconds: 120, memory: "512MB" }).https.onCall(async (data, context) => {
  if (!context.auth) {
  ...
  console.log("sendDocument - Payload recebido:", JSON.stringify({ phone, fileName, captionLength: caption?.length, conversationId, messageDocId, documentLength: document?.length }));
  ...
  const body: Record<string, string> = { phone, document, fileName };
